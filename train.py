...
random_seed = 123
model_save_path = 'efficientnetb3_grayscale_model.keras'
...
train_ds = tf.keras.utils.image_dataset_from_directory(
    ...
    color_mode='grayscale'  # 🔴 GÖRSELLER GRAYSCALE YÜKLENİYOR
)
...
# --- 3. Preprocessing Pipeline (gray → RGB + normalize) ---
def prepare_grayscale_to_rgb(x, y):
    x = tf.image.grayscale_to_rgb(x)              # ✅ EfficientNet için 3 kanala çevir
    x = preprocess_input(x)                       # ✅ normalize et (imagenet için)
    return x, y

AUTOTUNE = tf.data.AUTOTUNE
train_ds = train_ds.map(prepare_grayscale_to_rgb).cache().shuffle(1000).prefetch(AUTOTUNE)
val_ds = val_ds.map(prepare_grayscale_to_rgb).cache().prefetch(AUTOTUNE)
...
print(f"\n✅ Grayscale model başarıyla kaydedildi: {model_save_path}")
